...
function searchVehicles() {
    ...
    if (vehicleType === 'car' || vehicleType === 'rickshaw') {
        fetch(`https://saber-windy-nape.glitch.me/ride/api/vehicles?start=${encodeURIComponent(startCoords)}&end=${encodeURIComponent(endCoords)}&type=${vehicleType}`)
            .then(response => response.json())
            .then(data => {
...
function updateCapacity(vehicle_id, remaining_capacity) {
    fetch('https://saber-windy-nape.glitch.me/book/api/update-capacity', {
        ...
        .then(response => response.json())
        .then(data => {
            showMessage("The ride is booked successfully")
        })
        .catch(error => {
...
async function bookRide(vehicleId, fare, remaining_capacity) {
    ...
    try {
        ...
        const response = await fetch('https://saber-windy-nape.glitch.me/book/api/book-ride', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
...
function seats(remaining_capacity) {
    ...
    container.addEventListener("change", function (event) {
        ...
        showMessage("Your seats are set."); 
    });
