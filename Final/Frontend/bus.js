...
function searchVehicles() {
    ...
    fetch(`https://saber-windy-nape.glitch.me/bus/api/get-all-buses?lat=${encodeURIComponent(startLocation[0])}&lng=${encodeURIComponent(startLocation[1])}`)
        .then(response => {
...
driverId = driver_id
    ...
    fetch(`https://saber-windy-nape.glitch.me/bus/api/get-bus-stops?vehicle_id=${vehicle_id}`)
        .then(response => response.json())
        .then(data => {
...
async function getUserId(username) {
    try {
        const response = await fetch(`https://saber-windy-nape.glitch.me/auth/get-user-id/${encodeURIComponent(username)}`);
        if (!response.ok) {
...
function updateCapacity(vehicle_id, remaining_capacity) {
    ...
    fetch('https://saber-windy-nape.glitch.me/book/api/update-capacity', {
        method: 'POST',
        headers: {
...
async function bookRide(vehicle_id, fare, remaining_capacity) {
    ...
    try {
        const response = await fetch('https://saber-windy-nape.glitch.me/book/api/bus-book-ride', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
