...
document.getElementById('google-signin-btn').addEventListener('click', function (event) {
  ...
  auth.signInWithPopup(provider)
    .then(async (result) => {
      ...
      const response = await fetch('https://saber-windy-nape.glitch.me/auth/google-signin', {
        method: 'POST',
...
document.getElementById("signUpForm").addEventListener("submit", function (e) {
  ...
  fetch('https://saber-windy-nape.glitch.me/auth/signup', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify(data),
  })
...
document.getElementById('loginForm').addEventListener('submit', async function (event) {
  ...
  const response = await fetch('https://saber-windy-nape.glitch.me/auth/signin', {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
  ...
  {
    showMessage("Login Failed Unauthorized User");
  }
});
...
document.getElementById('google-signup-btn').addEventListener('click', function (event) {
  ...
  auth.signInWithPopup(provider)
    .then(async (result) => {
      ...
      fetch('https://saber-windy-nape.glitch.me/auth/check-user', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify(data),
      })
...
async function fetchUserData() {
  ...
  try {
    const response = await fetch('https://saber-windy-nape.glitch.me/auth/get-user-data', {
      method: 'GET',
      headers: {
    ...
    if (response.status === 403) {
      const refreshResponse = await fetch('https://saber-windy-nape.glitch.me/auth/refresh-token', {
        method: 'POST',
        headers: {
      ...
      if (refreshData.newToken) {
        sessionStorage.setItem('token', refreshData.newToken);
        fetchUserData();
      }
    } else {
      const data = await response.json();
